...
function processData() {
    ...
    logger.info(`Total de notes chargees : ${Object.keys(notesData).length}`);
    logger.info(`Total d'etudiants dans le fichier licences : ${Object.keys(licencesData).length}`);
    logger.info('');

    logger.info('Organisation des donnees par licence...');
    ...
    logger.success(`${totalTraites} etudiants traites avec succes`);
    logger.info(`${Object.keys(etudiantsParLicence).length} licences distinctes trouvees`);
    logger.info('');

    // Display detailed licence breakdown
    logger.info('Repartition par licence :');
    const licencesSorted = Object.keys(etudiantsParLicence).sort();
    for (const licence of licencesSorted) {
        const count = etudiantsParLicence[licence].length;
        logger.info(`  • ${licence} : ${count} etudiant(s)`);
    }
    logger.info('');

    if (etudiantsIgnores.length > 0) {
        logger.warning(`${etudiantsIgnores.length} etudiant(s) non trouve(s) dans le fichier licences :`);
        logger.info('');
        const etudiantsIgnoresTries = etudiantsIgnores.sort((a, b) => a.numero.localeCompare(b.numero));
        for (const e of etudiantsIgnoresTries) {
            logger.warning(`  • Numero CREM : ${e.numero} (Note : ${e.note.toFixed(2)})`);
        }
        logger.info('');
        logger.warning(`Ces etudiants seront places dans la feuille "Sans Licence"`);
        logger.info('');
    }
    ...
    for (const [groupName, licences] of Object.entries(groupes)) {
        if (licences.length > 0) {
            hasGroups = true;
            logger.info(`Configuration ${groupName} : ${licences.length} licence(s)`);
        }
    }
    if (hasGroups) logger.info('');

    logger.info('='.repeat(70));
    logger.info('GENERATION DU FICHIER EXCEL');
    logger.info('='.repeat(70));
    logger.info('');

    // Generate Excel file
    const outputFilename = document.getElementById('output-filename').value.trim() || 'resultats.xlsx';
    generateExcel(etudiantsParLicence, etudiantsIgnores, tauxReussite, groupes, outputFilename);

    // Update preview
    updatePreview(etudiantsParLicence, etudiantsIgnores);

    logger.info('');
    logger.info('='.repeat(70));
    logger.success('TRAITEMENT TERMINE AVEC SUCCES !');
    logger.info('='.repeat(70));
}
...
function generateExcel(etudiantsParLicence, etudiantsIgnores, tauxReussite, groupes, filename) {
    ...
    logger.success(`Feuille 'General' creee : ${allStudents.length} etudiants`);
    ...
    for (const [groupName, licences] of Object.entries(groupes)) {
        ...
        if (groupData.length > 0) {
            ...
            logger.success(`Feuille '${groupName}' creee : ${groupData.length} etudiants`);
        }
    }

    // Sheet "Sans Licence" for students not found in licences file
    if (etudiantsIgnores.length > 0) {
        const sansLicenceData = etudiantsIgnores
            .sort((a, b) => b.note - a.note)
            .map(e => ({
                'Numéro CREM': e.numero,
                'Note': e.note
            }));
        const wsSansLicence = XLSX.utils.json_to_sheet(sansLicenceData);
        XLSX.utils.book_append_sheet(wb, wsSansLicence, 'Sans Licence');
        logger.warning(`Feuille 'Sans Licence' creee : ${etudiantsIgnores.length} etudiants`);
    }

    // Download
    downloadXLSX(wb, filename);
    logger.success(`Fichier '${filename}' genere et telecharge avec succes`);
}

function updatePreview(etudiantsParLicence, etudiantsIgnores) {
    ...
    // Add row for students without licence
    if (etudiantsIgnores && etudiantsIgnores.length > 0) {
        const notes = etudiantsIgnores.map(e => e.note);
        html += `<tr style="background-color: var(--warning-bg, #fff3cd); color: var(--warning-text, #856404);">
            <td><strong>Sans Licence</strong></td>
            <td>${notes.length}</td>
            <td>${round(average(notes), 2)}</td>
            <td>${round(Math.min(...notes), 2)}</td>
            <td>${round(Math.max(...notes), 2)}</td>
